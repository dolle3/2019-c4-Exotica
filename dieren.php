...
<!--    dit zijn de reptielen-->
    ...
    if (isset($_POST['productcode']) && $_POST['productcode']!=""){
        ...
        $result = mysqli_query(
            $con,
            "SELECT * FROM `product` WHERE `productcode`= '$productcode' AND `productcategorieid` = 1"
        );
    ...
    $result = mysqli_query($con,"SELECT * FROM `product` WHERE `productcategorieid` = 1");
  ...
  <div class='image'><img src='".$row['image']."' height='167p'></div>
  <div class=\"card-body\">
    <div class='name'><h5>".$row['productnaam']."</h5></div>
    <h3><div class='price'>€".$row['prijs']."</div></h3>
    <button type='submit' class='button2' class= 'btn btn-primary'>IN WINKELMAAND</button>
    </div>
    </form>
  </div>
</div>
...
<section id="diensten2">
<!--    dit zijn de amfibien-->
<?php
    require_once "./core/db/connectdb.php";
    $status="";
    if (isset($_POST['productcode']) && $_POST['productcode']!=""){
    ...
    $result = mysqli_query($con,"SELECT * FROM `product` WHERE `productcategorieid` = 2");
    echo '<div class="container text-center"> <div class="row text-center">';
    while($row = mysqli_fetch_assoc($result)){
